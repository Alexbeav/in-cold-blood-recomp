...
#
# Writes: dist/In-Cold-Blood-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix In-Cold-Blood-Recomp \
  --exe-name In_Cold_Blood_Recompiled \
  --display-name "In Cold Blood Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned In Cold Blood disc" \
